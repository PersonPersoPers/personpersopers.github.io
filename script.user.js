...
(function() {
    ...
    GM_registerMenuCommand("Play FNAE", () => {
        if(popupMode == "page-replace"){document.head.innerHTML = "";document.documentElement.innerHTML = document.documentElement.innerHTML}
    ...
    GM_registerMenuCommand("Settings", () => {
        ...
        if(popupMode == "page-replace" || popupMode == "replace"){
            document.body.innerHTML = `
                <iframe style="position:absolute;top:0px;left:0px;width:100vw;height:100vh;z-index:2147483647;" src="https://personpersopers.github.io/settings.html"></iframe>
            `
        }
        else{
            document.body.innerHTML += `
    ...
    if(location.href == "https://personpersopers.github.io/settings.html"){
        ...
        document.body.innerHTML = `
            <div style="width:calc(100vw - 100px);height:100%;background-color:#333333;padding:50px;color:#ffffff;font-family:Arial;font-size:25px;">
                <label>Popup Method: </label>
                ...
                <div style="width:90vw;display:flex;">
                    <div style="width:60vw;margin-top:100px;" id="selector">
                    </div>
                    <div style="width:40vw;margin-top:100px;" id="view">
                        <select id="default-select" style="margin-botton:10px;">
                        </select><div></div>
                        <label>Name: </label>
                        <input type="text" id="name-rarity" style="margin-bottom:10px;"><div></div>
                        <label>Main Color: </label>
                        <input type="color" value="#7eef6d" id="main-color" style="margin-bottom:10px;"><div></div>
                        <label>Border Color: </label>
                        <input type="color" value="#66c258" id="border-color"><div></div><br>
                        <label style="font-size:25px;font-weight:800;">Fancy</label>
                        <input type="checkbox" id="fancy-active"><div></div>
                        <label>Hue: </label>
                        <input type="number" value="0" id="fancy-hue"><div></div>
                        <label>Saturation: </label>
                        <input type="number" value="100" id="fancy-sat"><div></div>
                        <label>Light: </label>
                        <input type="number" value="50" id="fancy-light"><div></div>
                        <label>Spread: </label>
                        <input type="number" value="0" id="fancy-spread"><div></div>
                        <label>Period: </label>
                        <input type="number" value="1.5" id="fancy-period"><div></div>
                        <label>Saturation Spread: </label>
                        <input type="number" value="0" id="fancy-sat-spread"><div></div>
                        <label>Light Spread: </label>
                        <input type="number" value="0" id="fancy-li-spread">
                    </div>
                </div>
            </div>
        `
        ...
        unsafeWindow.update = ({changeDefault=false, isSelect=false, changeValue=false}={}) => {
            var cd = flowrColors[unsafeWindow.selectedRarity]
            var index = flowrDefaultColors.findIndex(r => {
                return r.name == cd.name &&
                r.color == cd.color &&
                r.border == cd.border &&
                r.fancy?.hue == cd.fancy?.hue &&
                r.fancy?.sat == cd.fancy?.sat &&
                r.fancy?.light == cd.fancy?.light &&
                r.fancy?.satSpread == cd.fancy?.satSpread &&
                r.fancy?.liSpread == cd.fancy?.liSpread &&
                r.fancy?.period == cd.fancy?.period
            })
            var dcd = index != -1 ? flowrDefaultColors[index] : undefined
            if(dcd && !changeDefault){
                defaultSelector.value = index
            }
            if(fancyActive.checked == true){
                cd.fancy = {}
            }
            else{
                cd.fancy = undefined
            ...
            if(isSelect){
                var ncd = {}
                ncd.fancy = {}
                ncd.name = rarityName.value
                ncd.color = mainColor.value
                ncd.border = borderColor.value
                if(fancyActive.checked){
                    ncd.fancy.hue = fancyHue.value
                    ncd.fancy.sat = fancySat.value
                    ncd.fancy.light = fancyLight.value
                    ncd.fancy.spread = fancySpread.value
                    ncd.fancy.satSpread = fancySatSpread.value
                    ncd.fancy.liSpread = fancyLiSpread.value
                }
            }
            if(changeDefault){
                var ncd = flowrDefaultColors[parseInt(defaultSelector.value)]
                rarityName.value = ncd.name
                mainColor.value = ncd.color
                borderColor.value = ncd.border
                if(ncd.fancy){
                    fancyActive.checked = true
                    fancyHue.value = ncd.fancy?.hue ?? 0
                    fancySat.value = ncd.fancy?.sat ?? 100
                    fancyLight.value = ncd.fancy?.light ?? 50
                    fancySpread.value = ncd.fancy?.spread ?? 0
                    fancySatSpread.value = ncd.fancy?.satSpread ?? 0
                    fancyLiSpread.value = ncd.fancy?.liSpread ?? 0
                    fancyHue.disabled = false
                    fancySat.disabled = false
                    fancyLight.disabled = false
                    fancySpread.disabled = false
                    fancySatSpread.disabled = false
                    fancyLiSpread.disabled = false
                }
                else{
                    fancyActive.checked = false
                    fancyHue.disabled = true
                    fancySat.disabled = true
                    fancyLight.disabled = true
                    fancySpread.disabled = true
                    fancySatSpread.disabled = true
                    fancyLiSpread.disabled = true
                }
                flowrColors[unsafeWindow.selectedRarity] = ncd
            }
            else{
                var scd = flowrColors[unsafeWindow.selectedRarity]
                rarityName.value = scd.name
                mainColor.value = scd.color
                borderColor.value = cd.border
                if(scd.fancy){
                    fancyActive.checked = true
                    fancyHue.value = scd.fancy?.hue ?? 0
                    fancySat.value = scd.fancy?.sat ?? 100
                    fancyLight.value = scd.fancy?.light ?? 50
                    fancySpread.value = scd.fancy?.spread ?? 0
                    fancySatSpread.value = scd.fancy?.satSpread ?? 0
                    fancyLiSpread.value = scd.fancy?.liSpread ?? 0
                    fancyHue.disabled = false
                    fancySat.disabled = false
                    fancyLight.disabled = false
                    fancySpread.disabled = false
            ...
            for(var i = 0; i < 50; i++){
                var rarity = flowrColors[i]
                var dRarity = flowrDefaultColors[i]
                var color = rarity.color.replace("#", "")
                var r = parseInt(color.substring(0, 2), 16)
                var g = parseInt(color.substring(2, 4), 16)
                var b = parseInt(color.substring(4, 6), 16)
                var l = 0.2126 * r / 255 + 0.7152 * g / 255 + 0.0722 * b / 255
                if(!rarity.fancy){
                    colorSelector.innerHTML += `<button style="cursor:pointer;padding:10px;color:${l >= 0.179 ? "#000000" : "#ffffff"};background-color:${rarity.color};margin:10px;border-radius:20px;border-width:2px;border-color:${rarity.border};border-style:solid;font-weight:800;text-shadow: 0px 0px 1px #000;" onclick="selectedRarity=${i};update({isSelect: true});">${rarity.name}</button>`
                }
                else{
                    var fancy = rarity.fancy
                    var grad = gradientFlowr(fancy.hue, fancy.sat, fancy.light, fancy.spread, fancy.satSpread, fancy.liSpread)
                    colorSelector.innerHTML += `<button style="cursor:pointer;padding:10px;background:${grad[0]};margin:10px;border-radius:20px;border-width:2px;border-color:${rarity.border};border-style:solid;" onclick="selectedRarity=${i};update({isSelect: true});"><span style="background:${grad[1]};background-clip:text;color:transparent;font-weight:800;text-shadow: 1px 1px 2px #000;">${rarity.name}</span></button>`
                }
            }
            GM_setValue("flowrColors", JSON.stringify(flowrColors))
        }
        var colorSelector = document.getElementById("selector")
        var defaultSelector = document.getElementById("default-select"); defaultSelector.onchange = () => {unsafeWindow.update({changeDefault: true})}
        var rarityName = document.getElementById("name-rarity"); rarityName.onchange = () => {unsafeWindow.update({changeValue: true})}
        var mainColor = document.getElementById("main-color"); mainColor.onchange = () => {unsafeWindow.update({changeValue: true})}
        var borderColor = document.getElementById("border-color"); borderColor.onchange = () => {unsafeWindow.update({changeValue: true})}
        var fancyActive = document.getElementById("fancy-active"); fancyActive.onchange = () => {unsafeWindow.update({changeValue: true})}
        var fancyHue = document.getElementById("fancy-hue"); fancyHue.onchange = () => {unsafeWindow.update({changeValue: true})}
        var fancySat = document.getElementById("fancy-sat"); fancySat.onchange = () => {unsafeWindow.update({changeValue: true})}
        var fancyLight = document.getElementById("fancy-light"); fancyLight.onchange = () => {unsafeWindow.update({changeValue: true})}
        var fancySpread = document.getElementById("fancy-spread"); fancySpread.onchange = () => {unsafeWindow.update({changeValue: true})}
        var fancyPeriod = document.getElementById("fancy-period"); fancyPeriod.onchange = () => {unsafeWindow.update({changeValue: true})}
        var fancySatSpread = document.getElementById("fancy-sat-spread"); fancySatSpread.onchange = () => {unsafeWindow.update({changeValue: true})}
        var fancyLiSpread = document.getElementById("fancy-li-spread"); fancyLiSpread.onchange = () => {unsafeWindow.update({changeValue: true})}
        unsafeWindow.selectedRarity = 0
        for(var i = 0; i < 50; i++){
            var rarity = flowrColors[i]
            var dRarity = flowrDefaultColors[i]
            var color = rarity.color.replace("#", "")
            ...
            var l = 0.2126 * r / 255 + 0.7152 * g / 255 + 0.0722 * b / 255
            if(!rarity.fancy){
                colorSelector.innerHTML += `<button style="padding:10px;color:${l >= 0.179 ? "#000000" : "#ffffff"};background-color:${rarity.color};margin:10px;border-radius:20px;border-width:2px;border-color:${rarity.border};border-style:solid;font-weight:800;text-shadow: 0px 0px 1px #000;" onclick="selectedRarity=${i};update();">${rarity.name}</button>`
            }
            else{
                var fancy = rarity.fancy
                var grad = gradientFlowr(fancy.hue, fancy.sat, fancy.light, fancy.spread, fancy.satSpread, fancy.liSpread)
                colorSelector.innerHTML += `<button style="cursor:pointer;padding:10px;background:${grad[0]};margin:10px;border-radius:20px;border-width:2px;border-color:${rarity.border};border-style:solid;" onclick="selectedRarity=${i};update();"><span style="background:${grad[1]};background-clip:text;color:transparent;font-weight:800;text-shadow: 1px 1px 2px #000;">${rarity.name}</span></button>`
            }
            defaultSelector.innerHTML += `<option value="${i}">${dRarity.name}</option>`
        }
        function gradientFlowr(hue=0, sat=100, light=100, spread=30, satSpread=0, liSpread=0){
            var stop1 = `hsl(${hue-spread}, ${sat-satSpread}%, ${light-liSpread}%)`
            var stop2 = `hsl(${hue}, ${sat}%, ${light}%)`
            var stop3 = `hsl(${hue+spread}, ${sat+satSpread}%, ${light+liSpread}%)`
            function l(h, s, l) {
                s /= 100; l /= 100;
                const k = n => (n + h / 30) % 12
            ...
            var cstop1 = l(Math.abs(hue-spread)%360, clamp(sat-satSpread, 0, 100), clamp(light-liSpread, 0, 100)) >= 0.179 ? "#000000" : "#ffffff"
            var cstop2 = l(Math.abs(hue)%360, clamp(sat, 0, 100), clamp(light, 0, 100)) >= 0.179 ? "#000000" : "#ffffff"
            var cstop3 = l(Math.abs(hue+spread)%360, clamp(sat+satSpread, 0, 100), clamp(light+liSpread, 0, 100)) >= 0.179 ? "#000000" : "#ffffff"
            return [`linear-gradient(135deg, ${stop1}, ${stop2}, ${stop3})`, `linear-gradient(135deg, ${cstop1}, ${cstop2}, ${cstop3})`]
        }
    }
